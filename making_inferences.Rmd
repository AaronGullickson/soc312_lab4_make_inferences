...
Follow the instructions on Canvas to create a PDF from this R Markdown file that does several different hypothesis tests. You can use the R code chunks to show your work. Each test should be placed in a separate section with appropriate headers. You can use the template below to get started, but ultimately you should replace everything with the correct values and you should also remove these instructions.
...
mdiff <- 111.91304-90.06475
mdiff
tapply(movies$Runtime, movies$Genre, sd)
table(movies$Genre)
se <- sqrt(18.349683^2/207+9.344617^2/139)
se
tstat <- mdiff/se
tstat
2*pt(-tstat, 139-1)
